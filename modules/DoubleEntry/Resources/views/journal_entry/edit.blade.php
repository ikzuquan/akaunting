...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="row">
                {{ Form::dateGroup('paid_at', trans('general.date'), 'calendar', ['id' => 'paid_at', 'class' => 'form-control datepicker', 'required' => 'required', 'show-date-format' => company_date_format(), 'date-format' => 'Y-m-d', 'autocomplete' => 'off'], $journal->paid_at) }}

                {{ Form::textGroup('journal_number', trans_choice('general.numbers', 1), 'file', ['required' => 'required'], $journal->journal_number) }}

                {{ Form::textGroup('reference', trans('general.reference'), 'file-text-o', []) }}

                {{ Form::selectGroup('currency_code', trans_choice('general.currencies', 1), 'exchange-alt', $currencies, $journal->currency_code, ['required' => 'required', 'model' => 'form.currency_code', 'change' => 'onChangeCurrency']) }}
                {!! Form::hidden('currency_rate', $journal->currency_rate, ['id' => 'currency_rate', 'class' => 'form-control', 'required' => 'required']) !!}

                {{ Form::selectGroup('basis', trans('general.basis'), '', $basis, $journal->basis) }}

                {{ Form::textareaGroup('description', trans('general.description'), '', $journal->description, ['rows' => '3', 'required' => 'required']) }}

                {{ Form::fileGroup('attachment', trans('general.attachment'), '', ['dropzone-class' => 'w-100', 'multiple' => 'multiple', 'options' => ['acceptedFiles' => $file_types]], $journal->attachment, 'col-md-12') }}
